...
<script setup>
    ...
    const levelsCompleted = computed(() =>
        Math.max(0, userStore.currentLevel - 1)
    )
    const progressPercent = computed(() =>
        Math.round((levelsCompleted.value / totalLevels) * 100)
    )
    ...
    const languageFlag = computed(
        () => languageFlagMap[userStore.currentLanguage] || "un"
    )
